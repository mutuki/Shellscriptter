...
# xAuth bash Core version 1.1, optimaze for twitter.com version.
# Script written by Hiroshi Ogawa
# See more information http://www.kuropug.com/Shellscriptter/
# Last modify 10th April 2010
...
QUERYDATA="POST&https%3A%2F%2Fapi.twitter.com%2Foauth%2Faccess_token&$ENCREQUESTPAR"
...
if [ ${debug:-off} = "on" ]; then
echo "HTTP POST process start, strings are below. "
fi
 
ACCESSTOKEN=`curl --data oauth_consumer_key=$CKEY --data oauth_token=$AKEY --data oauth_nonce=$NONCEDATA --data oauth_timestamp=$TIMESTAMP --data oauth_signature_method="HMAC-SHA1" --data oauth_version="1.0" --data x_auth_mode="client_auth" --data x_auth_password=$UPASS --data x_auth_username=$UNAME --data oauth_signature=$HASHDATA --url "https://api.twitter.com/oauth/access_token"`
