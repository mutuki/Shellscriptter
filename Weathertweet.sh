...
echo -e "DAYLIGHT time information :\n$SUNRISE to $SUNSET"
	...
	if [ $SUNRISE -gt `date +%s` ] && [ `date +%s` -gt $SUNSET ]; then
		echo "will sunrise."
	else
		echo "will sunset."	
	fi	
...
if [ $SUNRISE -gt `date +%s` ] && [ `date +%s` -gt $SUNSET ]; then
	WEATHER=`echo "$WEATHER" | sed -e 's/2[0-9]*/⛈/g' -e 's/3[0-9]*/☂️/g' -e 's/5[0-9]*/☔️/g' -e 's/6[0-9]*/❄️/g' -e 's/7[0-9]*/🌫/g' -e 's/800/☀️/g' -e 's/801/🌤/g' -e 's/802/⛅️/g' -e 's/803/🌥/g' -e 's/804/☁️/g'| head -n 1`
else
	WEATHER=`echo "$WEATHER"	| sed -e 's/2[0-9]*/⛈/g' -e 's/3[0-9]*/☂️/g' -e 's/5[0-9]*/☔️/g' -e 's/6[0-9]*/❄️/g' -e 's/7[0-9]*/🌫/g' -e 's/8[0-9]*/🌕/g' | head -n 1`
fi
